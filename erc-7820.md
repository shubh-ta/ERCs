...
The `AccessControlRegistry` contract offers a standardized interface for managing access control, enhancing transparency and traceability. It emits key events such as `ContractRegistered` for contract registration, `ContractUnregistered` for contract removal, `RoleGranted` for granting roles, and `RoleRevoked` for revoking roles, ensuring all access control changes are clearly tracked.
...
The `register` function should be invoked either from the contract's constructor or its `initialize` function.
...
contract AccessControlRegistry is IAccessControlRegistry {
    ...
    // Unregisters a contract
    // @param _contract: Address of the contract to unregister
    ...
    // Grants roles to multiple accounts for multiple contracts
    // Roles are granted based on the tuple-like correspondence between arrays.
    // Each index of the arrays corresponds to a specific (contract, role, account) combination.
    // @param targetContracts: An array of contract addresses
    // @param roles: An array of roles to be granted
    // @param accounts: An array of accounts to be granted the roles
    function grantRole(
        address[] memory targetContracts,
    ...
    // Revokes roles from multiple accounts for multiple contracts
    // Roles are revoked based on the tuple-like correspondence between arrays.
    // Each index of the arrays corresponds to a specific (contract, role, account) combination.
    // @param targetContracts: An array of contract addresses
    // @param roles: An array of roles to be revoked
    // @param accounts: An array of accounts to be revoked the roles
    function revokeRole(
        address[] memory targetContracts,
        bytes32[] memory roles,
        address[] memory accounts
    ...
    // @return isActive status and admin address
